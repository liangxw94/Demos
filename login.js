function SSOController() {
    var undefined,
        ...
        ssoCrosssDomainUrl = 'https://login.sina.com.cn/sso/crossdomain.php',
        ssoLoginUrl = 'https://login.sina.com.cn/sso/login.php',
        ssoLogoutUrl = 'https://login.sina.com.cn/sso/logout.php',
        ssoUpdateCookieUrl = 'https://login.sina.com.cn/sso/updatetgt.php',
        ...
        generateVisitorUrl = 'https://passport.weibo.com/visitor/visitor',
    ...
    this.init = function() {
        ...
        me.isGenerateVisitor &&
            self === top &&
            Math.random() < me.generateVisitorProbability &&
            location.protocol !== 'https:' &&
            setTimeout(
                me.name + '.generateVisitor()',
                me.generateVisitorDelay * 1e3
            );
        me.isCheckLoginState &&
            addEventListener(window, 'load', function() {
                me.checkLoginState();
            });
        me.allowAutoFoundServerTime &&
            ssoLoginServerTime &&
    ...
    (function() {
        var a = function() {},
            ...
            c = { onComplete: a, onSuccess: a, onFailure: a },
            d = {
                vsnf: 'vsnf',
                vsnval: 'vsnval',
                door: 'door',
    ...
    this.login = function(a, b, c) {
        ...
        ssoLoginTimer.start(me.loginTimeout, function() {
            ...
            me.callbackLoginStatus({
                ...
                errno: -1,
                reason: unescape(
                    '%u767B%u5F55%u8D85%u65F6%uFF0C%u8BF7%u91CD%u8BD5'
                )
            });
        });
        c = c == undefined ? 0 : c;
    ...
    this.getServerTime = function(a, b) {
        if (me.servertime) {
            typeof b == 'function' &&
                b({ retcode: 0, servertime: me.servertime });
            return !0;
        }
    ...
    this.updateCookie = function() {
        try {
            if (me.autoUpdateCookieTime > 5) {
                ...
                updateCookieTimer = setTimeout(
                    me.name + '.updateCookie()',
                    me.autoUpdateCookieTime * 1e3
                );
            }
            ...
            a == null
                ...
                : a < b
                  ? (c = { retcode: 6203 })
                  : a - cookieExpireTimeLength + updateCookieTimeHardLimit > b
                    ? (c = { retcode: 6110 })
                    : a - b > me.noActiveTime && (c = { retcode: 6111 });
        ...
        try {
            ...
            if (d) {
                ...
                if (
                    loginMethodCheck &&
                    a.retcode == '2092' &&
                    me.allowAutoFoundServerTimeError
                ) {
                    me.setServerTime(0);
    ...
    this.crossDomainCallBack = function(a) {
        ...
        var b = a.arrURL,
            c,
            d,
            e = { callback: me.name + '.doCrossDomainCallBack' };
    ...
    this.crossDomainAction = function(a, b) {
        crossDomainTimer = setTimeout(
            me.name + '.crossDomainResult()',
            crossDomainTime * 1e3
        );
        typeof b == 'function'
            ? (crossDomainForward = b)
            : (crossDomainForward = null);
    ...
    this.appLogin = function(a, b, c) {
        var d = tmpData.savestate
                ? parseInt(
                      new Date().getTime() / 1e3 + tmpData.savestate * 86400
                  )
                : 0,
            e = getCookie('ALF') ? getCookie('ALF') : 0,
            f = { callback: c, ticket: a, ssosavestate: d || e },
            g = me.appLoginURL[b],
            h = makeURL(g, f);
    ...
    var makeNonce = function(a) {
        ...
        makeRequest = function(a, b, c, d) {
            ...
            if (
                ...
                e.nonce = me.nonce;
                e.pwencode = 'wsse';
                b = sinaSSOEncoder.hex_sha1(
                    '' +
                        sinaSSOEncoder.hex_sha1(sinaSSOEncoder.hex_sha1(b)) +
                        me.servertime +
                        me.nonce
                );
            }
        ...
        loginByXMLHttpRequest = function(a, b, c, d) {
            ...
            try {
                ...
                e.onreadystatechange = function() {
                    e.readyState == 4 &&
                        e.status == 200 &&
                        me.loginCallBack(parseJSON(e.responseText));
                };
        ...
        loginByScript = function(a, b, c, d) {
            ...
            e = objMerge(e, { callback: me.name + '.loginCallBack' });
        ...
        loginByIframe = function(a, b, c, d) {
            ...
            for (var g in me.loginExtraQuery) {
                if (typeof me.loginExtraQuery[g] == 'function') continue;
                f[g] = me.loginExtraQuery[g];
            }
            for (var h in f) e.addInput(h, f[h]);
            var i = makeURL(
                ssoLoginUrl,
                objMerge({ client: me.getClientType() }, me.loginExtraFlag)
            );
            e.method = 'post';
            e.action = i;
        ...
        parseJSON = function(str) {
            return typeof str == 'object'
                ? str
                : window.JSON ? JSON.parse(str) : eval('(' + str + ')');
        },
        ...
        try {
            if (me.shouldGenerateVisitor() && !me.$('visitorfrm84747h4784')) {
                document.body.insertAdjacentHTML(
                    'beforeEnd',
                    "<iframe id='visitorfrm84747h4784' style='position:absolute;left:0;top:0;border:none;width:1px;height:1px' src='" +
                        generateVisitorUrl +
                        "?from=iframe'/>"
                );
                setTimeout(function() {
...
(function() {
    ...
    var c = function(a, b) {
        ...
        e = function(a) {
            return a < 20
                ? 1518500249
                : a < 40 ? 1859775393 : a < 60 ? -1894007588 : -899497514;
        },
        f = function(a, b) {
    ...
    this.base64 = {
        ...
        _subp_v2_keys:
            'uAL715W8e3jJCcNU0lT_FSXVgxpbEDdQ4vKaIOH2GBPtfzqsmYZo-wRM9i6hynrk=',
        _subp_v3_keys_3:
            '5WFh28sGziZTeS1lBxCK-HgPq9IdMUwknybo.LJrQD3uj_Va7pE0XfcNR4AOYvm6t'
    };
    this.Cookie = {
        decode: function(a) {
            var b = [],
            ...
            switch (c) {
                case 'v01':
                    ...
                    return decodeURIComponent(
                        j(sinaSSOEncoder.base64.decode(b, 'binnary', 'array'))
                    );
                case 'v02':
                    d = d.replace(/\./g, '=');
                    b = sinaSSOEncoder.base64.decode(d, 'urlsafe', 'array');
                    return j(
                        sinaSSOEncoder.base64.decode(b, 'binnary', 'array')
                    );
                default:
                    return decodeURIComponent(a);
            }
        ...
        decode: function(a) {
            ...
            switch (d) {
                ...
                case '003':
                    c = e.substr(0, 1);
                    e = e.substr(1);
                    b = sinaSSOEncoder.base64.decode(
                        e,
                        'subp_v3_' + c,
                        'array'
                    );
