!(function($win, $doc) {
    ...
    STK.register('core.util.browser', function($) {
        ...
        var ret = {
            ...
            IE9: /msie 9/.test(ua),
            SAFARI:
                !/chrome\/([\d.]*)/.test(ua) && /\/([\d.]*) safari/.test(ua),
    ...
    STK.register('core.util.URL', function($) {
        return function(sURL, args) {
            ...
            that.valueOf = that.toString = function() {
                var url = [];
                var query = $.core.json.jsonToQuery(
                    query_json,
                    opts.isEncodeQuery
                );
                var hash = $.core.json.jsonToQuery(
                    hash_json,
                    opts.isEncodeQuery
                );
                if (url_json.scheme != '') {
    ...
    STK.register('common.widget.log', function($) {
        ...
        return function(opts) {
            var conf = $.core.obj.parseParam(
                { vsrc: 'app_weiboshow', refer: '', step: 1 },
                opts
            );
            var refer = scope.refer || scope.$refer || conf.refer,
                uid = scope.loginKit().uid || '',
                appid = scope.appkey || $CONFIG.$appkey || $CONFIG.appkey || 0,
                cat = CATEGARY[conf.vsrc] || '';
            var src =
                '//rs.sinajs.cn/r.gif?uid=' +
                uid +
                '&appid=' +
                appid +
                '&refer=' +
    ...
    STK.register('comp.widget.followButton.login', function($) {
        var lang = scope.language ? scope.language : 'zh_cn';
        var url =
            'http://' +
            location.host +
            '/dialog/follow.php?fuid=' +
            scope.uid +
            '&refer=' +
            scope.refer +
            '&language=' +
            lang +
            '&type=widget_page&vsrc=app_followbutton&backurl=' +
            encodeURIComponent(document.referrer) +
            '&rnd=' +
            new Date().valueOf();
        if (/service/.test(location.host)) {
            url =
                'http://' +
                location.host +
                '/widget/dialog/follow.php?fuid=' +
                scope.uid +
                '&refer=' +
                scope.refer +
                '&language=' +
        ...
        return function() {
            var showLogin = function(cbfun) {
                App.loginBackUrlCallBack = function() {
                    scope.logined = 1;
                    cbfun && cbfun();
                    $.common.widget.log({ vsrc: 'app_followbutton', step: 2 });
                };
                scope.postLogin = 1;
                var loginPopWindow = window.open(
                    url,
                    'miniblog_login',
                    [
                        'toolbar=0,status=0,resizable=1,width=620,height=540,left=',
                        (screen.width - 620) / 2,
                        ',top=',
                        (screen.height - 450) / 2
                    ].join('')
                ...
                App.loginPopWindow = loginPopWindow;
                $.common.widget.log({ vsrc: 'app_followbutton' });
            };
            return showLogin;
        };
    });
    ...
    scope.loginKit = function() {
        if (window.scope) {
            var uid =
                window.scope.current_user_weibo ||
                window.scope.current_user_sina;
            if (uid) {
                return { uid: uid, isLogin: !!uid };
            }
        }
        if (window.$CONFIG) {
            var uid =
                window.$CONFIG.current_user_weibo ||
                window.$CONFIG.current_user_sina ||
                window.$CONFIG.uid ||
                window.$CONFIG.$uid;
            if (uid) {
                return { uid: uid, isLogin: !!uid };
        ...
        uid = uid ? uid[2] || '' : '';
        var oid = scope['$oid'];
        return { uid: uid, isLogin: !!uid, isAdmin: uid && oid && uid == oid };
    };
    ...
    STK.register('comp.widget.followButton.ssologin', function($) {
        var analytics = function(status) {
            status = typeof status == 'undefined' ? -1 : status;
            var uid = scope.loginKit().uid;
            new Image().src =
                '//rs.sinajs.cn/b.gif?uid=' +
                uid +
                '&refer=' +
                encodeURIComponent(scope.refer) +
                '&url=' +
                encodeURIComponent(document.referrer) +
                '&followed=' +
                status +
        ...
        var isfansUrl = '//widget.weibo.com/relationship/aj_isfans.php';
        if (/service/.test(location.host)) {
            isfansUrl =
                '//' + location.host + '/widget/relationship/aj_isfans.php';
            if (/jssdk/.test(location.href)) {
                isfansUrl =
                    '//' + location.host + '/widget/jssdk/aj_isfans.php';
            }
        }
        return function(callback) {
            (function() {
                this.entry = 'wbwidget';
                this.service = 'miniblog';
                this.domain = 'weibo.com';
                this.appLoginURL['weibo.com'] =
                    'http://weibo.com/sso/login.php';
    ...
    STK.register('comp.widget.followButton.lang', function($) {
        return function() {
            var sysLang = {
                    zh_cn: {
                        n001: '已关注',
                ...
                Lang = sysLang[scope.language ? scope.language : 'zh_cn'];
            return Lang;
        };
    });
    STK.register('common.widget.analytics', function($) {
        var prtl =
                document.location.protocol.indexOf('https') > -1
                    ? 'https://'
                    : 'http://',
            domain = 'beacon.sina.com.cn';
        var SUDA_URL = prtl + domain + '/e.gif';
        var STATIC_URL = prtl + 'rs.sinajs.cn/social.gif';
        return function(opts) {
            var that = {};
            var conf = $.core.obj.parseParam(
                {
            ...
            var params =
                ...
                '||&gUid_' +
                new Date().valueOf();
            var sudaUrl = SUDA_URL + params;
            var staticUrl = STATIC_URL + params;
            var SUDA_IMG = new Image();
            SUDA_IMG.src = sudaUrl;
            var SUDA_IMG = new Image();
            SUDA_IMG.src = staticUrl;
            return that;
        };
    });
    var $ = STK;
    var sCore = $.core,
        login = $.comp.widget.followButton.login(),
        ssologin = $.comp.widget.followButton.ssologin,
        removeEvent = sCore.evt.removeEvent,
        addEvent = sCore.evt.addEvent,
        stopEvent = sCore.evt.stopEvent,
        ajax = sCore.io.ajax;
    var followUrl =
            'http://' + location.host + '/relationship/aj_attention.php',
        isfansUrl = 'http://widget.weibo.com/relationship/aj_isfans.php';
    if (/service/.test(location.host)) {
        followUrl =
            'http://' + location.host + '/widget/relationship/aj_attention.php';
        isfansUrl =
            'http://' + location.host + '/widget/relationship/aj_isfans.php';
    }
    var Lang = $.comp.widget.followButton.lang();
    var $followBtn = $.E('followBtn'),
        $fStatus = $.E('fStatus'),
        $fText = $.E('fText'),
        $fNum = $.E('fNum');
    var fClass = $fStatus.className,
        lock = 0;
    var status = {
        init: function(cls, txt, title) {
            cls = cls ? fClass + ' ' + cls : fClass;
            txt && ($fText.innerHTML = txt);
            $fStatus.className = cls;
            if (title) {
                $fStatus.title = title;
            }
        },
        load: function() {
            this.init('WB_Bloading', null, '');
        },
        followed: function(txt, title) {
            this.init('WB_followed', txt, title);
            $followBtn.setAttribute('href', 'http://weibo.com/u/' + scope.uid);
            $followBtn.setAttribute('target', '_blank');
            addEvent($followBtn, 'click', log);
        }
    };
    var log = function() {
        var SUDA_IMG = new Image();
        SUDA_IMG.src = '//rs.sinajs.cn/social.gif?followbutton=1';
    };
    var error = function() {
        status.init('', Lang.n005);
    };
    var numWrite = function() {
        if ($fNum && scope.write == 1) {
            var fans = $fNum.innerHTML;
            fans = parseInt(fans, 10) + 1;
            if (!isNaN(fans)) {
                $fNum.innerHTML = fans;
            }
        }
    ...
    var callback = function(json) {
        lock = 0;
        if (json.code == '0') {
            status.init(null, null, Lang.n006 + scope.uname);
            return;
        }
        if (
            json.code === 'A00006' ||
            ...
            unbindDOM();
            status.followed(Lang.n001, Lang.n002 + scope.uname);
        } else {
            if (json.code === 'M00006' || json.code == 3) {
                unbindDOM();
                status.followed(Lang.n004, Lang.n004);
            } else {
                if (json.code === 'M00005') {
                    if (!json.firstAjax) {
                        login(addFollow);
    ...
    var suda = function() {
        var URL = $.core.util.URL(document.location.href);
        var social = URL.getParam('social');
        var cur_uid = scope.loginKit().uid || 0,
            _uid = scope.uid || 0;
        if (social) {
            $.common.widget.analytics({
                uid: cur_uid,
                aext:
                    'followclk:' +
                    cur_uid +
    ...
    var addFollow = function(e) {
        ...
        suda();
        if (scope.logined != 1 && !scope.loginKit().isLogin) {
            login(addFollow);
            return;
        }
        lock = 1;
        var data = {
            ...
            wsrc: $CONFIG.wsrc || 'app_follow_button',
            uid: scope.uid
        };
        var nodeData = $followBtn.getAttribute('action-data');
        if (!nodeData) {
            return;
        }
        var nodeJson = $.core.json.queryToJson(nodeData);
        lock = 0;
        nodeJson.uid &&
            window.open(
                'http://www.weibo.com/u/' +
                    nodeJson.uid +
                    '?refer_flag=9549880000_guanzhuanniu'
            );
    };
    var bindDOM = function() {
            addEvent($followBtn, 'click', addFollow);
        },
        unbindDOM = function() {
            removeEvent($followBtn, 'click', addFollow);
        };
    bindDOM();
    ssologin(callback);
    window.App = window.App || {};
    window.App.doFollow = addFollow;
    window.App.loginBackUrlCallBack = function() {};
})(window, document);
